...
// Function to generate PDF order details
...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    // Get userId from the first item
    ...
    if (paymentMethod === 'stripe') {
      try {
        ...
        const lineItems = items.map(item => {
          ...
          let imageUrl = '';
          if (item.image) {
            // If image is a relative path, convert to absolute URL
...
export const handleStripeWebhook = async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      // Get the order details before deleting cart items
      const orderDetails = {

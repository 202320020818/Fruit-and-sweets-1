  ...
  export default function ShoppingCart() {
    ...
    // Handle item quantity change (increase/decrease)
    ...
    const handleCheckout = async () => {
      ...
      try {
        ...
        if (session.id) {
          // Before redirecting to Stripe, clear cart items in the frontend
          setCartItems([]); // Empty the cart immediately on the frontend
          ...
          // After payment is successful, confirm payment on the backend
          ...
          if (confirmationResult.success) {
            // Optional: Redirect to the payment success page after payment confirmation
                    ...
                    <Row gutter={16} align="middle" style={{ width: "100%" }}>
                      ...
                      <Col span={10}>
                        ...
                        <Tooltip title="Remove item">
                          <Button
                            ...
                            danger
                            icon={<DeleteOutlined />}
                            onClick={() => handleDelete(item.itemId)} // Call handleDelete on button click
                          />
                        </Tooltip>
                      ...
                      <Col span={6}>
                        <InputNumber
                          ...
                          value={item.quantity}
                          style={{ width: "60px" }}
                          onChange={(value) => handleQuantityChange(item.itemId, value)} // Handle quantity change
                        />
                        <Button
                          icon={<MinusOutlined />}
                          style={{ marginLeft: "5px" }}
                          onClick={() => handleQuantityChange(item.itemId, Math.max(item.quantity - 1, 1))} // Decrease quantity
                        />
                        <Button
                          icon={<PlusOutlined />}
                          style={{ marginLeft: "5px" }}
                          onClick={() => handleQuantityChange(item.itemId, item.quantity + 1)} // Increase quantity
                        />
                      </Col>
                      <Col span={4}>
